...
- **Create** the file in Phase 1 as soon as the plan file's name is fixed: `runId`, `owner_session: $CLAUDE_CODE_SESSION_ID`, `updated_at`, `baseRef` (the `git rev-parse HEAD` captured before Step 1 spawns the Leader — the first docs of the run are written after it), `phase: "P1"`, all `retries` zeroed, `globalCycle: 1`.

...
3. Read current counters from `.claude/session-state/runs/<plan-id>.json` (never from recall), find the matching row in `escalation.md`'s Phase Transition Table, and apply its `To` target + `Counter` effect; write the updated counters back to the file
...
6. Report to user: `⚠ ESCALATION: [source] → [target]. Reason: [reason]. Retry: N/3. Global cycle: N/3.` — values read from `runs/<plan-id>.json`, not recalled
